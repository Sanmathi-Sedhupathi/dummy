  ...
  const fetchStats = async () => {
    try {
      ...
      const stats = {
        ...
        totalEarnings: completedOrders.reduce((sum: number, b: any) => sum + (b.pilot_earnings || 0), 0)
      };
      setStats(stats);
    } catch (err) {
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-sm font-medium text-green-600">
                        {formatCurrency(order.pilot_earnings)}
                      </div>
                      <div className="text-xs text-gray-500">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-6xl shadow-lg rounded-md bg-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                <h4 className="font-semibold text-gray-900 border-b pb-2">Requirements & Features</h4>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700">Editor Assigned</label>
                  <p className="mt-1 text-sm text-gray-900">{selectedOrder.editor_name || 'Not assigned yet'}</p>
                  {selectedOrder.editor_email && (
                    <p className="text-xs text-gray-500">{selectedOrder.editor_email}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <h5 className="font-medium text-green-900 mb-2">My Earnings (50%)</h5>
                  <div className="text-2xl font-bold text-green-600">
                    {formatCurrency(selectedOrder.pilot_earnings)}
                  </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex justify-between">
                      <span>Pilot (Me):</span>
                      <span className="font-semibold text-green-600">{formatCurrency(selectedOrder.pilot_earnings)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Editor:</span>
              ...
              {orders.map((order) => (
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-gray-500">
                      50% of {formatCurrency(order.final_cost)}
                    </div>
                  </td>
...
const FinalReviewContent: React.FC = () => {
  ...
  const fetchFinalReviewOrders = async () => {
    try {
      ...
      // Filter for orders ready for final review
      const reviewOrders = (response.data || []).filter((order: any) => 
            ...
            <div key={order.id} className="border rounded-lg p-4">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-medium">Order HMX{order.id.toString().padStart(4, '0')}</h3>
                  <p className="text-sm text-gray-500">Client: {order.client_name}</p>
                  <p className="text-sm text-gray-500">Earnings: {formatCurrency(order.pilot_earnings)}</p>
                </div>
                <div className="flex space-x-2">
